...
EVALUATOR_HASH = hashlib.sha256(b"evaluator-v1").hexdigest()
...
def test_wrong_key_or_evaluator_fails_preflight_without_consuming_holdout(tmp_path):
    ...
    wrong_key_runner = FinalHoldoutRunner(
        registry=registry,
        seals=seals,
        keys=StaticKeys(key=OTHER_KEY),
        ledger=ledger,
        evaluator=CountingEvaluator(),
    )
    with pytest.raises(HoldoutRunError):
    ...
    wrong_evaluator_runner = FinalHoldoutRunner(
        registry=registry,
        seals=seals,
        keys=StaticKeys(),
        ledger=ledger,
