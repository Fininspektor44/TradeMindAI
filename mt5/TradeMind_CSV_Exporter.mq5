...
#property description "Read-only MT5 candle exporter for TradeMind AI"

input string          InpSymbols       = "XAUUSD,EURUSD,GBPUSD";
input ENUM_TIMEFRAMES InpTimeframe     = PERIOD_M5;
input int             InpBarsToExport  = 300;
...
bool ExportSymbol(string symbol)
{
   ...
   {
      PrintFormat("TradeMind exporter: cannot select symbol %s", symbol);
      return false;
   }
   ...
   // Start from shift 1 so only closed candles are exported.
   int copied = CopyRates(symbol, InpTimeframe, 1, InpBarsToExport, rates);
   if(copied < InpBarsToExport)
   {
      PrintFormat(
         "TradeMind exporter: %s %s requested=%d copied=%d. Wait for history to load.",
         symbol,
         TimeframeLabel(),
         InpBarsToExport,
         copied
   ...
   {
      PrintFormat("TradeMind exporter: FileOpen failed for %s, error=%d", filename, GetLastError());
      ResetLastError();
      return false;
   }
...
void ExportAll()
{
   ...
   if(count <= 0)
   {
      Print("TradeMind exporter: symbol list is empty");
      return;
   }
   ...
   PrintFormat(
      "TradeMind exporter: exported %d/%d symbols to Common\\Files\\%s",
      exported,
      count,
      InpOutputFolder
   );
...
int OnInit()
{
   if(InpBarsToExport < 60)
   {
      Print("TradeMind exporter: InpBarsToExport must be at least 60");
      return INIT_PARAMETERS_INCORRECT;
   }
   if(InpRefreshSeconds < 1)
   {
      Print("TradeMind exporter: InpRefreshSeconds must be at least 1");
      return INIT_PARAMETERS_INCORRECT;
   }
