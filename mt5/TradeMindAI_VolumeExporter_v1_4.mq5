...
#property description "Exports closed-bar microstructure only. It never sends trading orders."

input string          InpSymbols        = "XAUUSD,XAGUSD,.USTECHCash,.US500Cash,.US30Cash,WTI,BRENT";
input ENUM_TIMEFRAMES InpTimeframe      = PERIOD_M5;
input int             InpTimerSeconds   = 10;
input int             InpBackfillBars   = 72;
input int             InpCatchupBars    = 288;
...
string OutputFile(const string symbol)
{
   return InpOutputFolder+"\\volume_"+SafeSymbolName(symbol)+"_"+TimeframeText(InpTimeframe)+".csv";
}
...
int OpenOutput(const string symbol)
{
   string path=OutputFile(symbol);
   ResetLastError();
   int handle=FileOpen(path,
      FILE_READ|FILE_WRITE|FILE_CSV|FILE_ANSI|FILE_SHARE_READ|FILE_COMMON,',');
   if(handle==INVALID_HANDLE)
   {
      Print("TradeMind v1.4: cannot open ",path," error=",GetLastError());
   ...
   double &percentile_100)
{
   ...
   int copied=CopyRates(symbol,InpTimeframe,from_time,to_time,history);
   if(copied<=0)
      return;
...
bool AppendBar(const string symbol,const MqlRates &bar)
{
   ...
   if(copied<0)
      ...
      copy_status="NO_TICKS";
   else if(copy_error!=0)
      copy_status="PARTIAL";

   int tick_count=(copied>0 ? copied : 0);
   int bid_up=0,bid_down=0,ask_up=0,ask_down=0;
   int mid_up=0,mid_down=0,buy_ticks=0,sell_ticks=0;
   ulong trade_volume=0;
   double trade_volume_real=0.0;
   double previous_bid=0.0,previous_ask=0.0,previous_mid=0.0;
   double spread_sum=0.0,spread_min=0.0,spread_max=0.0;
   double spread_first=0.0,spread_last=0.0;
   ...
   double direction_imbalance=(directional_ticks>0
      ? (double)(mid_up-mid_down)/(double)directional_ticks : 0.0);
   ...
   HistoricalVolumeMetrics(symbol,bar.time,bar.tick_volume,rvol_20,volume_percentile_100);

   int handle=OpenOutput(symbol);
   if(handle==INVALID_HANDLE)
      return false;
   ...
   FileWrite(handle,
      ...
      DoubleToString(spread_max,6),
      DoubleToString(spread_last,6),
      DoubleToString(spread_expansion,6),
      DoubleToString(realized_abs_move_points,6),
      DoubleToString(direction_imbalance,8),
      delta_proxy,
      DoubleToString(rvol_20,8),
      DoubleToString(volume_percentile_100,6),
      DoubleToString(range_per_tick,8),
      DoubleToString(body_per_tick,8),
      copy_status,
      copy_error);
   FileFlush(handle);
   FileClose(handle);
   ...
   if(InpDebugLog)
      Print("TradeMind v1.4 ",symbol," ",TimeframeText(InpTimeframe),
            " bar=",TimeToString(bar.time,TIME_DATE|TIME_MINUTES),
            " ticks=",tick_count," rvol=",DoubleToString(rvol_20,2),
            " status=",copy_status);
...
void BackfillSymbol(const int index)
{
   ...
   int copied=CopyRates(symbol,InpTimeframe,1,InpBackfillBars,rates);
   if(copied<=0)
   {
      Print("TradeMind v1.4: no backfill bars for ",symbol," error=",GetLastError());
      return;
   }
   ...
   {
      AppendBar(symbol,rates[i]);
      if(rates[i].time>latest)
         latest=rates[i].time;
   }
...
{
   string symbol=g_symbols[index];
   datetime latest_closed=iTime(symbol,InpTimeframe,1);
   if(latest_closed<=0 || latest_closed<=g_last_exported[index])
      return;

   MqlRates rates[];
   ArraySetAsSeries(rates,false);
   int copied=CopyRates(symbol,InpTimeframe,1,InpCatchupBars,rates);
   if(copied<=0)
      return;
   ...
   for(int i=0;i<copied;i++)
   {
      if(rates[i].time<=g_last_exported[index])
         continue;
      bool ok=AppendBar(symbol,rates[i]);
      if(!ok)
         break;
...
int OnInit()
{
   ...
   {
      Print("TradeMind v1.4: invalid input values");
      return INIT_PARAMETERS_INCORRECT;
   }
   g_bar_seconds=PeriodSeconds(InpTimeframe);
   if(g_bar_seconds<=0)
   {
      Print("TradeMind v1.4: unsupported timeframe");
      return INIT_PARAMETERS_INCORRECT;
   }
   int count=ParseSymbols(InpSymbols,g_symbols);
   if(count<=0)
   {
      Print("TradeMind v1.4: no symbols configured");
      return INIT_PARAMETERS_INCORRECT;
   }
   ...
   for(int i=0;i<count;i++)
   {
      ...
      {
         Print("TradeMind v1.4: cannot select symbol ",g_symbols[i]);
         continue;
      }
      BackfillSymbol(i);
   }
   EventSetTimer(InpTimerSeconds);
   Print("TradeMind AI v1.4 Volume Intelligence started. Read-only. Symbols=",count,
...
{
   EventKillTimer();
   Print("TradeMind AI v1.4 Volume Intelligence stopped. reason=",reason);
}
...
void OnTick()
{
   // Intentionally empty. The exporter is timer-driven and never sends orders.
}
