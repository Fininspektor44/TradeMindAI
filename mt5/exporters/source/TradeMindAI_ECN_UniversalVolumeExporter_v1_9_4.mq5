...
#property description "TradeMind AI v1.9.4 universal read-only ECN volume exporter"
...
input string          InpManifestFile     = "ecn_manifest.csv";
...
bool IsCryptoCanonical(const string canonical)
{
   string normalized=NormalizeSymbol(canonical);
   return (normalized=="BTCUSD" || normalized=="ETHUSD");
}

string RowSchema(const string canonical)
{
   return (IsCryptoCanonical(canonical) ? "1.7" : "1.4");
}
...
int ParseSymbols(const string list,string &symbols[])
{
   ...
   for(int i=0;i<count;i++)
   {
      ...
      {
         if(NormalizeSymbol(symbols[j])==normalized)
         {
            duplicate=true;
            break;
         }
...
int OpenOutput(const string canonical)
{
   ...
   {
      Print("TradeMind v1.9.4: cannot open ",path," error=",GetLastError());
...
void WriteManifest(string &requested[])
{
   ...
   if(handle==INVALID_HANDLE)
   {
      Print("TradeMind v1.9.4: cannot write manifest ",path," error=",GetLastError());
      return;
   }
   FileWrite(handle,"schema_version","canonical_symbol","broker_symbol","status","timeframe","source_id");
   for(int i=0;i<ArraySize(requested);i++)
   {
      string actual=ResolveBrokerSymbol(requested[i]);
      FileWrite(handle,RowSchema(requested[i]),requested[i],actual,
         (actual=="" ? "MISSING" : "RESOLVED"),TimeframeText(InpTimeframe),"ROBO_ECN");
   }
   FileFlush(handle);
   FileClose(handle);
}

void HistoricalVolumeMetrics(const string actual,const datetime bar_time,const long current_volume,
   double &rvol_20,double &percentile_100)
{
   rvol_20=0.0;
   percentile_100=0.0;
...
bool AppendBar(const string actual,const string canonical,const MqlRates &bar)
{
   ...
   if(handle==INVALID_HANDLE)
      return false;
   FileWrite(handle,
      RowSchema(canonical),(long)bar.time,canonical,TimeframeText(InpTimeframe),g_bar_seconds,
   ...
   FileClose(handle);
   if(InpDebugLog)
      Print("TradeMind v1.9.4 ",canonical," <- ",actual," bar=",TimeToString(bar.time,TIME_DATE|TIME_MINUTES),
...
void BackfillSymbol(const int index)
{
   ...
   if(copied<=0)
   {
      Print("TradeMind v1.9.4: no backfill bars for ",g_canonical[index],
            " actual=",g_actual[index]," error=",GetLastError());
      return;
   }
   datetime latest=0;
...
int OnInit()
{
   ...
   {
      Print("TradeMind v1.9.4: invalid input values");
      return INIT_PARAMETERS_INCORRECT;
   }
   g_bar_seconds=PeriodSeconds(InpTimeframe);
   if(g_bar_seconds<=0)
   {
      Print("TradeMind v1.9.4: unsupported timeframe");
      return INIT_PARAMETERS_INCORRECT;
   }
   string requested[];
   ...
   {
      Print("TradeMind v1.9.4: no ECN symbols configured");
      return INIT_PARAMETERS_INCORRECT;
   }
   ...
   for(int i=0;i<requested_count;i++)
   {
      ...
      if(actual=="")
      {
         Print("TradeMind v1.9.4: ECN symbol not found: ",requested[i]);
         continue;
      }
      if(!SymbolSelect(actual,true))
      {
         Print("TradeMind v1.9.4: cannot select broker symbol ",actual," for ",requested[i]);
         continue;
      }
      ...
      Print("TradeMind v1.9.4: resolved ",requested[i]," -> ",actual);
   }
   WriteManifest(requested);
   int count=ArraySize(g_actual);
   if(count<=0)
   {
      Print("TradeMind v1.9.4: no ECN symbols resolved in this terminal");
      return INIT_FAILED;
   }
   ...
   Print("TradeMind AI v1.9.4 ECN Universal Volume started. Read-only. Resolved=",count,
         " requested=",requested_count," folder=",InpOutputFolder," source=ROBO_ECN");
...
{
   EventKillTimer();
   Print("TradeMind AI v1.9.4 ECN Universal Volume stopped. reason=",reason);
}
