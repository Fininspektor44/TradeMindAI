...
Реальный сбой на Windows: v1.3 корректно создавал
`mt5_risk_orders_utc_67206924.csv` и `mt5_risk_deals_utc_67206924.csv`,
но оба файла содержали только заголовок -- ни одной строки. Реальные
тикеты `733124339` и `733124518` точно существовали и были исполнены.
Автоматическая сверка поэтому корректно возвращала `AMBIGUOUS`
(тикеты отсутствуют в экспорте истории) -- это НЕ ошибка сверки, а
следствие того, что история туда вообще не попала.

**Подтверждённая первопричина**: `HistorySelect(from, to)` в MQL5
принимает интервал в BROKER/SERVER TIME. Код v1.3 использовал
`TimeGMT()` для обеих границ (`from`/`to`) -- `TimeGMT()` возвращает GMT,
а не время сервера брокера. На сервере, опережающем GMT (обычный случай
для большинства брокеров), собственная серверная метка времени недавнего
ордера может оказаться ПОЗЖЕ верхней границы, построенной на `TimeGMT()`,
и такой ордер полностью выпадает из выбранного диапазона -- отсюда
пустая (только заголовок) история при реально существующих и
исполненных тикетах.

**Исправление**: `ExportOrderHistorySnapshot`/`ExportDealHistorySnapshot`
теперь строят обе границы `HistorySelect` через `TimeCurrent()`
(документированное время сервера брокера), а не `TimeGMT()`. Обе границы
используют один и тот же базис. Ничего другое не изменено: `UtcNowMsc()`
(время записи в CSV) и таймерный каденс (`g_last_risk_snapshot_at`,
`OnTimer()`) по-прежнему используют `TimeGMT()` -- это исправление
затрагивает исключительно интервал запроса `HistorySelect`. Магический
фильтр (`InpMagicNumber`), ограниченное окно `InpHistoryLookbackDays` и
исполнение ордеров (`ProcessPendingRequest`) не изменены.

Добавлена явная диагностика: если `HistorySelect` возвращает `false`,
обе функции теперь пишут `Print`/`PrintFormat` с именем экспорта и
`GetLastError()`, вместо того чтобы неотличимо молча писать пустую
историю. Успешный результат с нулём строк (`HistorySelect` вернул
`true`, но подходящих сделок/ордеров действительно нет) по-прежнему
считается валидным и не логируется как ошибка.
