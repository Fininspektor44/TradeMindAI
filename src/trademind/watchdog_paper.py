"""TradeMind v1.8 watchdog extension for the forward paper signal gate."""
...
from trademind.paper_gate_v18 import VALID_GATE_STATUSES
...
from trademind.watchdog_unified import run_watchdog_unified

SCHEMA_VERSION = "1.8.0"


def inspect_paper_states(path: Path) -> tuple[WatchdogCheck, dict[str, int]]:
    counts: dict[str, int] = {}
    if not path.is_file():
        return WatchdogCheck("Paper gate states", "ERROR", f"missing file: {path}"), counts
    rows = 0
    malformed = 0
    try:
        with path.open("r", encoding="utf-8-sig", newline="") as handle:
            for row in csv.DictReader(handle):
                rows += 1
                status = str(row.get("gate_status") or "").strip().upper()
                if status not in VALID_GATE_STATUSES:
                    malformed += 1
                    continue
                counts[status] = counts.get(status, 0) + 1
    except OSError as exc:
        return WatchdogCheck("Paper gate states", "ERROR", f"invalid CSV: {exc}"), counts
    if rows == 0:
        return WatchdogCheck("Paper gate states", "ERROR", "no gate state rows"), counts
    if malformed:
        return (
            WatchdogCheck(
                "Paper gate states",
                "ERROR",
                f"{malformed} rows with invalid gate status",
...
def run_watchdog_paper(
    *,
    ...
    snapshot = run_watchdog_unified(
        ...
        unified_states_path=unified_states_path,
        unified_dashboard_path=unified_dashboard_path,
        task_snapshot_path=task_snapshot_path,
        status_path=status_path,
        report_path=report_path,
        html_path=html_path,
        crypto_manifest_path=crypto_manifest_path,
    ...
    checks.extend(
        (
            _file_check("Paper gate signals", paper_signals_path, derived_max_age_minutes, now),
            _file_check("Paper gate states", paper_states_path, derived_max_age_minutes, now),
            _file_check("Paper gate decisions", paper_decisions_path, derived_max_age_minutes, now),
            _file_check("Paper gate dashboard", paper_dashboard_path, derived_max_age_minutes, now),
        )
    )
    paper_check, paper_counts = inspect_paper_states(paper_states_path)
    checks.append(paper_check)

    counts = dict(snapshot.validation_counts)
    counts.update({f"PAPER_{name}": value for name, value in paper_counts.items()})
    overall = _overall(checks)
    notify_required = overall == "ERROR" and previous != "ERROR"
    paths = dict(snapshot.paths)
    paths.update(
        {
            "paper_signals": str(paper_signals_path),
            "paper_states": str(paper_states_path),
            "paper_decisions": str(paper_decisions_path),
    ...
    _atomic_text(
        ...
        _render_text(snapshot).replace(
            "TradeMind AI v1.4.4 Watchdog", "TradeMind AI v1.8.0 Watchdog"
        ),
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check TradeMind v1.8 paper gate pipeline")
    ...
    parser.add_argument(
        "--paper-signals", type=Path, default=Path("data/paper_signal_gate_v1_8/signals.csv")
    )
    parser.add_argument(
        "--paper-states", type=Path, default=Path("data/paper_signal_gate_v1_8/latest.csv")
    )
    parser.add_argument(
        "--paper-decisions",
        type=Path,
        default=Path("data/paper_signal_gate_v1_8/decisions.csv"),
    )
    parser.add_argument(
        "--paper-dashboard",
        type=Path,
        default=Path("data/paper_signal_gate_v1_8/dashboard/index.html"),
    )
    ...
    try:
        snapshot = run_watchdog_paper(
            ...
            paper_signals_path=args.paper_signals.expanduser().resolve(),
            paper_states_path=args.paper_states.expanduser().resolve(),
            paper_decisions_path=args.paper_decisions.expanduser().resolve(),
            paper_dashboard_path=args.paper_dashboard.expanduser().resolve(),
    ...
    print("TradeMind AI v1.8.0 Watchdog")
